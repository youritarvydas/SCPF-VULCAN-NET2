...
app.get("/api/profile", async (req, res) => {
	try {
		...
		const groupsWithIcons = await getGroupIcons(
			groups.map(group => ({ group: { id: group.id } }))
		)

		for (const group of groups) {
			group.icon = groupsWithIcons.get(String(group.id)) || null
		}

		const mainGroupId = configuredGroupIds[0]

		const mainGroup = groups.find(
			group => String(group.id) === String(mainGroupId)
		)

		let allies = []

		if (mainGroup) {
			allies = await getGroupAllies(mainGroup.id)

			const allyIcons = await getGroupIcons(
				allies.map(group => ({ group: { id: group.id } }))
			)

			for (const ally of allies) {
				ally.icon = allyIcons.get(String(ally.id)) || null
			}
		}
